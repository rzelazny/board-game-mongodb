...
function checkUsableBuildings(gameData) {
    ...
    if(beforeProd){
        gameData.players.forEach(player => {
            ...
            if (player.constructedBuildings.some(e => e.name === "Chapel").length > 0) {
                ...
                if (diceSum < 8 && player.dice.length > 2) {
                    canUseBuilding = playerCanUseBuilding(player, "Chapel", canUseBuilding, player.dice);
                }
            }
        ...
        gameData.players.forEach(player => {
            ...
            let mintFlag = false;
            if (player.constructedBuildings.some(e => e.name === "Mint").length > 0) {
                //Mint modifies when other buildings can be used
                mintFlag = true;
            }
    
            //see if they've buit a Sawmill
            if (player.constructedBuildings.some(e => e.name === "Sawmill").length > 0 && (mintFlag || season2)) {
                //Sawmill triggers end of Summer unless the mint has been built
                canUseBuilding = playerCanUseBuilding(player, "Sawmill", canUseBuilding);
            }
            //see if they've buit a Quarry
            if (player.constructedBuildings.some(e => e.name === "Quarry").length > 0 && (mintFlag || season2)) {
                //Quarry triggers end of Summer unless the mint has been built
                canUseBuilding = playerCanUseBuilding(player, "Quarry", canUseBuilding);
            }
            //see if they've buit a Goldsmith
            if (player.constructedBuildings.some(e => e.name === "Goldsmith").length > 0 && (mintFlag || season2)) {
                //Goldsmith triggers end of Summer unless the mint has been built
                canUseBuilding = playerCanUseBuilding(player, "Goldsmith", canUseBuilding);
            }
            //see if they've buit a Inn
            if (player.constructedBuildings.some(e => e.name === "Inn").length > 0 && season2){
                //Inn triggers end of Summer
                canUseBuilding = playerCanUseBuilding(player, "Inn", canUseBuilding);
            }
            //see if they've buit a Merchants' Guild
            if (player.constructedBuildings.some(e => e.name === "Merchants' Guild").length > 0){
                //Merchants' Guild triggers end of every productive season
                canUseBuilding = playerCanUseBuilding(player, "Merchants' Guild", canUseBuilding);
            }
            //see if they've buit a Town Hall
            if (player.constructedBuildings.some(e => e.name === "Town Hall").length > 0){
                //Town Hall triggers end of every productive season
                canUseBuilding = playerCanUseBuilding(player, "Town Hall", canUseBuilding);
            }
            //see if they've buit an Embassy
            if (player.constructedBuildings.some(e => e.name === "Embassy").length > 0){
                //Embassy triggers end of every productive season
                canUseBuilding = playerCanUseBuilding(player, "Embassy", canUseBuilding);
            }
            //see if they've buit a Training Camp
            if (player.constructedBuildings.some(e => e.name === "Training Camp").length > 0 && season3){
                //Training Camp triggers end of season 3
                canUseBuilding = playerCanUseBuilding(player, "Training Camp", canUseBuilding);
            }
...
function pickAdvisors() {
    ...
    mongo.getGame(curGame, ((gameData) => {
        mongo.getPlayer(gameData.players[curPlayer - 1], (playerData) => {
            ...
            marketFlag = (playerData.constructedBuildings.some(e => e.name === "Market").length > 0)
