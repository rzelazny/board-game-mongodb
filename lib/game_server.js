...
function checkUsableBuildings(gameData) {
    ...
    const example = async (gameData) => {
        const players = gameData.players;
        for (const player of players) {
            const result = await returnNum(player);
            console.log(result);
            //Prompt the player to use any available buildings
            if (Object.keys(result).length > 0) {
                promptUser(result.player, "use-buildings", result);
    ...
    const returnNum = player => {
        return new Promise((resolve, reject) => {
            ...
            resolve(canUseBuilding);
        });
    }
    example(gameData).then(() => {
        console.log("Ready to move on, when is this running?");
    ...
    //         //Prompt the player to use any available buildings
    //         if (Object.keys(canUseBuilding).length > 0) {
    ...
    //     });
    // }
    // console.log("test when this is running");
    // //if someone has buildings to use start the phase timer, otherwise move on
    // if (waitingForPlayers > 0) beginPhaseTimeout(10, "Use buildings expired");
    // else nextPhase();
}
...
async function playerCanUseBuilding(player, building, canUseObj, choiceType, dice = []) {
    ...
    if (Object.keys(canUseObj).length === 0) {
        ...
        canUseObj = { player: player._id, building: [building], choice: [choiceType], dice: dice }
    }
