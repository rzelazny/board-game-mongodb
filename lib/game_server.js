...
function playGame(passedPhase = false) {
    console.log("passedPhase", passedPhase);
    //if a phase is passed in, use it instead of the current global phase
    //this happens when a phase repeatedly due to turn order
    
    if(passedPhase === "Advisor"){
        curPhase = 4;
    }
...
function influenceAdvisors(player, choice){
    ...
    mongo.getPlayer(player, (playerData) => {
        ...
        else if (!advisors.includes(total)) { //if the advisor hasn't already been picked
            //mark advisor
            advisors.push(total);
