const { getAdvisor } = require("./db-updates");
...
//function makes sure updates are validate and reverts them if not
function validateData(player) {
    console.log("validating data for", player);
    mongo.getPlayer(player, ({ resource1, resource2, resource3 }) => {
        ...
        //if a resource is negative adv 6 was used. Revert the conversion
        if (resource1 < 0) {
            console.log("res 1 is negative");
            updateData.res1 = 1;
            updateData.res2 = -1;
            updateData.res3 = -1;
            mongo.updatePlayerResources(updateData, sidebarUpdate)
        }
        else if (resource2 < 0) {
            console.log("res 2 is negative");
            updateData.res1 = -1;
            updateData.res2 = 1;
            updateData.res3 = -1;
            mongo.updatePlayerResources(updateData, sidebarUpdate)
        } else if (resource3 < 0) {
            console.log("res 3 is negative");
            updateData.res1 = -1;
            updateData.res2 = -1;
            updateData.res3 = 1;
            mongo.updatePlayerResources(updateData, sidebarUpdate)
...
function choiceProcessed() {
    ...
    if (waitingForPlayers === 0) {
        ...
        // if(curPlayer > players.length || curPhase === 1){//phase1 doesn't have a current player
        curPlayer = 1;
        nextPhase();
    }
    else {
...
function pickAdvisors() {
    ...
    mongo.getGame(curGame, ((gameData) => {
        mongo.getPlayer(gameData.players[curPlayer - 1], (playerData) => {
            ...
            promptUser(playerData._id, "update-dice", diceData, false);
        })
    }))
...
//function checks who still has dice, and removes player that passed.
function playersWithDice({ players }, passed) {
...
function advisorResources(player, choice) {
    ...
    for (let i = 0; i < choice.length; i++) {
        ...
        mongo.getAdvisor({ number: advisor }, (advData) => {
            ...
            mongo.updatePlayerResources(updateData, validateData);
        })
    }
}
