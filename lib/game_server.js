...
function rewardPhase(phase) {
    ...
    //update the winning player's off player's bonus status in the db
    mostBuildingPlayers.forEach(player => {
        let update = { player: player, field: "score", data: score+1 };
        mongo.updatePlayer(update, () => { });
    });

    promptUser(winners, "prompt-user", "You've gained a victory point from the King's reward.");
