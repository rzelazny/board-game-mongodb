...
function playGame() {
    ...
    switch (curPhase) {
        ...
        case 7:
            return rewardPhase(curPhase);
        case 8:
            return postProdCleanup(); //Summer
        case 9:
...
function productivePhase(phase) {
    ...
    mongo.getGame(curGame, buildingCheckBeforeProd);
}
...
function buildingCheckBeforeProd(gameData) {
        ...
        choice.forEach(building => {
            switch (building) {
                case "Chapel": //reroll all dice
                    let newDice = (generateDie(3));
                    mongo.updatePlayer({ player: playerData._id, field: "dice", data: newDice }, blank); //this was push
            ...
            mongo.getBuilding(choice, (buildingData) => {
                ...
                if (playerData.resource1 >= costRes1 &&
                    ...
                    mongo.updatePlayerResources(playerData, () => {
                        mongo.updatePlayer({ player: player, field: "constructedBuildings", data: buildingData[0]._id }, () => { //this was push
                            waitingForPlayers--;
                            waitingForDB--;
