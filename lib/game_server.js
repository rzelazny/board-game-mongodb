...
exports.twoOutputs = function (gameName, db, cb) {
    let result = {}
    db.Game.findById(gameName)
        ...
        .then(gameData => {
            ...
            console.log("Players with data: ", players);
            console.log("Got data, ready to start game");
            result = {
                id1: players[0].socket, 
                id2: players[1].socket
            }
...
exports.initGame = function (/*sio, socket,*/ sdb, gameName) {
    ...
    getGameState(gameName, playGame);
...
function nextPhase() {
    ...
    if (curPhase === 8) {
        ...
        if (curRound === 5) endGame();
            ...
            curRound++;
            curPhase = 1;
            mongo.updateGame(currentGame, "curPhase", curPhase);
            mongo.updateGame(currentGame, "curRound", curRound);
        }
    }
    else {
        curPhase++;
...
function aidPhaseOne() {
    ...
    if (leastBuildingPlayers.length > 1) {
        ...
        console.log("test ", leastBuildingPlayers);
...
function promptUser(updatePlayer, prompt) {
    ...
    players.forEach(player => {
        if (updatePlayer.includes(player._id)) {
            ...
            if(gameSocket === player.socket){
                c
                console.log("messaging myself");
                gameSocket.emit(prompt, { message: "This is a test" });
            }
            else{
                console.log("messaging others");
                gameSocket.to(player.socket).emit(prompt, "test");
            } 
            // io.on("connection", (socket) => {

            //     // sending to the client
            //     socket.to(player.socket).emit(prompt, "I just met you");
            // })
        }
        else {
            ...
            if(gameSocket === player.socket){
                gameSocket.emit("waiting", { message: "Waiting for other players" });
            }
            else gameSocket.to(player.socket).emit("waiting", "Waiting for other players");
        }
    });
}
