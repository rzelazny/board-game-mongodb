...
function playGame() {
    ...
    switch (curPhase) {
        ...
        case 14:
            return postProdCleanup();
        case 15:
            return aidPhaseTwo(); //Aid Phase Two
...
function checkUsableBuildings(gameData) {
    ...
    const usableBuildings = async (gameData) => {
        ...
        for (const player of players) {
            ...
            if (Object.keys(hasUsableBuildings).length > 0) {
                promptUser(hasUsableBuildings.player, "use-buildings", hasUsableBuildings);
    ...
    const findUsable = player => {
        return new Promise((resolve, reject) => {
            ...
            playerCanUseBuilding(player, canUseBuilding, gameData.buildings, (data)={
                resolve(data);
            })
