...
receiveChoice = function ({ player, choiceType, choice }) {
    ...
    switch (choiceType) {
        case "aidResource":
            ...
            break;
        case "advisor":
            influenceAdvisors(player, choice);
            break;
        case "building":
            resolveBuildings(player, choice);
            break;
        default:
    ...
    gameData.players.forEach(player => {
        ...
        if (player.constructedBuildings.some(e => e.name === "Statue").length > 0) {
            ...
            if (player.dice[0] === player.dice[1] && player.dice[1] === player.dice[2]) {
                playerCanUseBuilding(player, "Statue", canUseBuilding, player.dice);
        ...
        //if (player.constructedBuildings.some(e => e.name === "Chapel").length > 0) {
            ...
            //if (diceSum < 8 && player.dice.length > 2) {
                playerCanUseBuilding(player, "Chapel", canUseBuilding, player.dice); //can always use chapel for testing purposes
            //}
       // }
        console.log(canUseBuilding);
...
function playerCanUseBuilding(player, building, canUseArray, dice){
    ...
    //building test
    //let timeVar = "";
    //promptUser(gameData.players[0]._id, "prompt-user", "Would you like to use your Chapel?");
    //buildingTimeout(gameData.players[0]._id, 10, "Chapel prompt timed out", timeVar);
    //awaitBuildingResponse++;
...
// function buildingTimeout(player, timer, msg, timeVar) {

//     timeVar = setTimeout(() => {
//         console.log(msg);
//         awaitBuildingResponse--;
//         //receiveChoice(player, "building", "timeout");
//     }, timer * 1000);

//     promptUser(player, "timer", timeVar);
...
async function prepTurnOrder (cb) {
    let diceSumArray = [];
...
function PickAdvisors(playerDiceData) {
    console.log("Select advisors in turn order");
    console.log(playerDiceData);
    ...
    beginPhaseTimeout(30, ("Player number", i, "selected their advisor"));
        ...
        choice.forEach(building => {
            switch(building){
                case "Chapel": //reroll all dice
                    ...
                    console.log(player, "has new dice");
                break;
            default:
