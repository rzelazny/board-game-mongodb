...
async function aBuildingCheckBeforeProd(gameData) {
    ...
    //see who has relevant buildings
    gameData.players.forEach(player => {
        if (player.constructedBuildings.some(e => e.name === "Statue").length > 0) {
            hasStatue.push(player)
        }

        if (player.constructedBuildings.some(e => e.name === "Market").length > 0) {
            hasMarket.push(player)
        }

        //see if the buildings apply
        hasMarket.forEach(player => {
        ...
        hasMarket.forEach(player => {
            ...
            if (diceSum < 8) {
                promptUser(player, "prompt-user", "Would you like to use your Market?");
            }
        })
    });
...
function setTurnOrder() {
    ...
    //turn each player's dice array into a single number
    players.forEach(player => {
        console.log(player.dice[0]);
        let total = player.dice[0].reduce((a, b) => { return a + b })
        console.log(player.dice);
        let playerTotal = { player: player._id, name: player.name, color:player.color, total: total }
